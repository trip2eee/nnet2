...
def get_dot_graph(output, verbose=True):
    ...
    add_module(output.creator)
    txt += _dot_tensor(output, verbose)

    while modules:
        mod = modules.pop()
        txt += _dot_module(mod)
        for x in mod.inputs:
            txt += _dot_tensor(x, verbose)
...
def _dot_tensor(t: nnet.Tensor, verbose=False):
    dot_var = '{} [label="{}", color=orange, style=filled]\n'

    name = '' if t.name is None else t.name
    if verbose and t.data is not None:
...
def _dot_module(mod: Function):
    ...
    dot_edge = '{} -> {}\n'
    for x in mod.inputs:
        txt += dot_edge.format(id(x), id(mod))
    
    for y in mod.outputs:
        txt += dot_edge.format(id(mod), id(y()))
    return txt
